...
#if defined(NDEBUG)
# error "KhmerDigital cannot be compiled without assertions."
#endif
...
#define REWARD_CUT_BLOCK 3961000
...
uint256 hashGenesisBlock("0x92cf7be938f2cbc37315bf17acc8869c6d6559f7e21c47b1bc1ec2f340b9fdfa"); //hash genesis block miannet
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // KhmerDigital: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "KhmerDigital Signed Message:\n";
...
{
    // KhmerDigital: IsDust() detection disabled, allows any valid dust to be relayed.
...
static const int64 nTargetTimespan = 24 * 60 * 60; // KhmerDigital: 1 Day
static const int64 nTargetSpacing = 2 * 60; // KhmerDigital: 2 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // KhmerDigital: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // KhmerDigital: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet) // KhmerDigital increments value by 2
    {
        pchMessageStart[0] = 0xfa;
        pchMessageStart[1] = 0xc4;
        pchMessageStart[2] = 0xb9;
        pchMessageStart[3] = 0xdf;
        hashGenesisBlock = uint256("0x579e0f024aade4ac284ff194f1a2d24351cf7bfae835ce58018ef1a2859a68f4");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "Khmer Digital 03/03/2019";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime    = 1551631429;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 164498;

        if (fTestNet)
        {
            block.nTime    = 1551631445;
            block.nNonce   = 57038;
        }
        ...
        assert(block.hashMerkleRoot == uint256("0x690818bfb61ff7742a84fac829d0ec54bf74bc08b5b81428152893ddc3a9ae92"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xfd, 0xc2, 0xb9, 0xda }; // KhmerDigital: increase each by adding 2 to litecoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("KhmerDigitalMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("KhmerDigitalMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static KhmerDigitalMiner(CWallet *pwallet)
{
    printf("KhmerDigitalMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("KhmerDigital-miner");
    ...
    try { loop {
        ...
        printf("Running KhmerDigitalMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("KhmerDigitalMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&KhmerDigitalMiner, pwallet));
}
